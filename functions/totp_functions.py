...
from bonus_functions.password_functions import ask_password

def hotp(key, counter, digits=6, digest='sha256'):
...
def totp(key, time_step=30, digits=6, digest='sha256'):
    ...
    with open('__data__/token', 'rb') as keyfile:
        token = keyfile.read()
    key = "__data__/"+key
    return hotp(decrypt(key, token), int(time.time() / time_step), digits, digest)
